...
import loginCover from '../assets/auth-cover.jpg';
...
const Login = () => {
  ...
  const { signInWithGoogle, user: supabaseUser, loading: authLoading } = useSupabaseAuth();

  // Redirect if already authenticated (either traditional or Supabase)
  useEffect(() => {
    if (isAuthenticated || supabaseUser) {
      const from = location.state?.from?.pathname || '/dashboard';
      navigate(from, { replace: true });
    }
  }, [isAuthenticated, supabaseUser, navigate, location]);
    ...
    try {
      ...
      if (result.success) {
        ...
        try {
          ...
          // User doesn't have preferences, go to preferences form
      ...
      <div className="w-full lg:w-1/2 bg-white flex flex-col justify-center px-6 sm:px-8 lg:px-12 py-8 lg:py-0">
        ...
        <div className="max-w-sm mx-auto w-full mt-12 sm:mt-16 lg:mt-0">
          <div className="mb-4 sm:mb-6">
            <h1 className="text-2xl sm:text-3xl font-light text-gray-900 mb-2">Welcome Back</h1>
          </div>
