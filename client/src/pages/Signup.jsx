...
const Signup = () => {
  ...
  const { signInWithGoogle, user: supabaseUser, loading: authLoading, upsertProfile } = useSupabaseAuth();

  // Redirect if already authenticated (traditional) or handle Supabase auth
  useEffect(() => {
    if (isAuthenticated) {
    ...
    // If user signed in via Supabase (Google), upsert profile and send to preferences
    if (supabaseUser) {
      (async () => {
        try {
          await upsertProfile({
          ...
          // swallow profile errors but log
          console.error('Profile upsert error:', e);
        }

        // After sign in with Google, send user to preferences to complete onboarding
        navigate('/preferences', { replace: true });
      })();
    }
  }, [isAuthenticated, supabaseUser, upsertProfile, navigate]);

  // Trigger Supabase Google sign-in
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Supabase will handle redirect; auth state listener will call upsertProfile
    } catch (err) {
        ...
        <div className="max-w-sm mx-auto w-full mt-12 sm:mt-16 lg:mt-0">
          ...
          <div className="mt-3 sm:mt-4">
            <button 
              ...
              className="w-full bg-white border border-gray-200 text-gray-700 py-2.5 sm:py-3 px-4 rounded-lg font-medium hover:bg-gray-50 transition-all duration-200 flex items-center justify-center text-sm disabled:opacity-50 disabled:cursor-not-allowed">
